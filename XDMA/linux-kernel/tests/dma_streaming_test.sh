...
for ((i=1; i<$channelPairs; i++))
do
  rm -f data/output_datafile_32M.bin
  echo "Info: DMA setup to read from c2h channel $i. Waiting on write data to channel $i."
  $tool_path/dma_from_device -d /dev/xdma0_c2h_${i} -f data/output_datafile_32M.bin -s $transferSize -c $transferCount &
done
...
do
  ...
  $tool_path/dma_to_device -d /dev/xdma0_h2c_${i} -f data/datafile_32M.bin -s $transferSize -c $transferCount &
done
...
do
  ...
  cmp data/output_datafile_32M.bin data/datafile_32M.bin -n $transferSize
  returnVal=$?
  if [ ! $returnVal == 0 ]; then
    echo "Error: The data written did not match the data that was read."
    echo "       write data file: data/datafile_32M.bin"
    echo "       read data file:  data/output_datafile_32M.bin"
    testError=1
  else
